...
// //SEARCHCSVBYCOLNAME
//TODO:
test('Searching by column name', async ({ page }) => {
  await page.goto('http://localhost:8000/');
  await page.getByLabel('Login').click();
  await page.getByPlaceholder('Enter command here!').click();
  await page.getByPlaceholder('Enter command here!').fill('load_file <movies.csv>');
...
test('Searching by column num', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Submit' }).click();
  await expect(page.getByRole('cell', { name: 'Eternal Sunshine of the' })).toBeVisible();
  await expect(page.getByRole('cell', { name: 'Michel Gondry' })).toBeVisible();
  await expect(page.getByRole('cell', { name: '2004' })).toBeVisible();
  await expect(page.getByRole('cell', { name: '8.3' })).toBeVisible();
  await expect(page.getByRole('cell', { name: '8.3' })).toBeVisible();
  await expect(page.locator('table').filter({ hasText: 'Eternal Sunshine of the' })).toBeVisible();
